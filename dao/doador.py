    ...
    try:
        ...
        with conexao.cursor() as cursor:
            cursor.execute("""
                ...
                ("CPF_ID", nome, email, contato, senha, status_conta, cargo, data_criacao) 
...
# Verifica se o doador está ativo
...
def excluir_doador(cpf_id):
    try:
        ...
        with conexao.cursor() as cursor:
            ...
            conexao.commit()
            if cursor.rowcount:
                return True, "Doador excluído com sucesso."
            else:
                return False, "Doador não encontrado."
...
# DELETE - Excluir doador em cascata
def excluir_doador_completa(cpf_id):
    try:
        conexao = criar_conexao()
        if not conexao:
            return False, "Falha ao conectar ao banco."
        ...
        with conexao.cursor() as cursor:
            # Excluir feedbacks
            cursor.execute('DELETE FROM public.feedback WHERE id_doador = %s', (cpf_id,))
            
            # Excluir itens de pedidos
            cursor.execute('''
                DELETE FROM public.pedido_item 
                WHERE "ID_Pedido" IN (
                    SELECT "ID_Pedido" FROM public.pedido WHERE "ID_Doador" = %s
                )
            ''', (cpf_id,))
            
            # Excluir pedidos
            cursor.execute('DELETE FROM public.pedido WHERE "ID_Doador" = %s', (cpf_id,))
            
            # Excluir itens de intenção
            cursor.execute('''
                DELETE FROM public.intencao_item 
                WHERE "ID_Intencao" IN (
                    SELECT "ID_Intencao" FROM public.intencaodoacao WHERE "ID_Doador" = %s
                )
            ''', (cpf_id,))
            
            # Excluir intenções
            cursor.execute('DELETE FROM public.intencaodoacao WHERE "ID_Doador" = %s', (cpf_id,))
            
            # Excluir doador
            cursor.execute('DELETE FROM public."doador" WHERE "CPF_ID" = %s', (cpf_id,))

            conexao.commit()
            return True, "Doador e dados vinculados excluídos com sucesso."
