...
def gerenciar_listas_ong():
    ...
    mapa_itens = {
        i["ID_Item"]: f"{i['categoria']} - {i['subcategoria'] or 'Sem subcategoria'}"
        for i in catalogo
    }
    ...
    # Itens + feedbacks por lista
    for lista in listas_ong:
        lista["itens"] = exibir_lista_itens(lista["ID_Lista"])
        feedbacks = buscar_feedbacks_por_lista(lista["ID_Lista"])
        lista["likes"] = sum(1 for f in feedbacks if f.get("reacao") is True)
        lista["dislikes"] = sum(1 for f in feedbacks if f.get("reacao") is False)
