...
# Configuração do Mailtrap (coloque isso no app principal)
...
def atualizar_senha_usuario(email, nova_senha):
    ...
    try:
        with conexao.cursor() as cursor:
            ...
            if cursor.fetchone():
                cursor.execute('UPDATE public."ong" SET senha = %s WHERE email = %s',
                               (senha_criptografada, email.strip().lower()))
                conexao.commit()
            ...
            if cursor.fetchone():
                cursor.execute('UPDATE public."doador" SET senha = %s WHERE email = %s',
                               (senha_criptografada, email.strip().lower()))
                conexao.commit()
