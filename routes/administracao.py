...
from dao.ong import listar_ongs, atualizar_status_ong, atualizar_status_verificacao_ong, excluir_ong_completa
from dao.doador import listar_doadores, atualizar_status_doador, excluir_doador_completa
...
def painel_admin():
    ...
    if request.method == "POST":
        ...
        if acao == "aprovar_ong":
            ...
            sucesso, msg = excluir_ong_completa(id)
            ...
            sucesso, msg = excluir_doador_completa(id)
            ...
            if not categoria:
                flash("A categoria é obrigatória.")
    ...
    # GET - Carregar dados
    ongs = listar_ongs()
    doadores = [d for d in listar_doadores() if d.get("cargo") != 1]
    itens = exibir_itens()
