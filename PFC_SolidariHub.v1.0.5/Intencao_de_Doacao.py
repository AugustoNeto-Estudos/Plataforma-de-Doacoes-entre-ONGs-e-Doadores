...
def inserir_intencao(id_intencao, id_ong, id_doador, id_lista, status, data_criacao):
    try:
        ...
        conexao.commit()
        return True, str("✅ Intenção inserida com sucesso!")
    except Error as erro:
        return False, str(f"❌ Erro ao inserir intenção: {str(erro).strip()[:200]}")
    finally:
        try:
            cursor.close()
            conexao.close()
...
def listar_intencoes():
    try:
        ...
        st.error(str(f"❌ Erro ao listar intenções: {str(erro).strip()[:200]}"))
        return []
    finally:
        try:
            cursor.close()
...
def atualizar_intencao(id_intencao, id_ong, id_doador, id_lista, status, data_criacao):
    try:
        ...
        cursor.execute(sql, valores)
        conexao.commit()
        return True, str("✅ Intenção atualizada com sucesso!")
    except Error as erro:
        return False, str(f"❌ Erro ao atualizar intenção: {str(erro).strip()[:200]}")
    finally:
        try:
            cursor.close()
            conexao.close()
...
def atualizar_status(id_intencao, novo_status):
    try:
        ...
        conexao.commit()
        return True, str("✅ Status atualizado com sucesso!")
    except Error as erro:
...
def deletar_intencao(id_intencao):
    try:
        ...
        conexao.commit()
        if cursor.rowcount:
            return True, str("✅ Intenção deletada com sucesso!")
        else:
            return False, str("⚠️ Intenção não encontrada.")
    except Error as erro:
        return False, str(f"❌ Erro ao deletar intenção: {str(erro).strip()[:200]}")
    finally:
        try:
            cursor.close()
            conexao.close()
...
if st.button("📥 Carregar intenções"):
    ...
    if dados:
        ...
        st.info(str("Nenhuma intenção encontrada."))
