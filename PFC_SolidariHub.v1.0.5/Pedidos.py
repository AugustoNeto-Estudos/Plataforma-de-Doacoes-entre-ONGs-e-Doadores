...
def inserir_pedido(id_pedido, id_ong, id_doador, id_intencao, status, data_criacao):
    try:
        ...
        conexao.commit()
        return True, str("✅ Pedido inserido com sucesso!")
    except Error as erro:
        return False, str(f"❌ Erro ao inserir pedido: {str(erro).strip()[:200]}")
    finally:
        try:
            cursor.close()
            conexao.close()
...
def listar_pedidos():
    try:
        ...
        st.error(str(f"❌ Erro ao listar pedidos: {str(erro).strip()[:200]}"))
        return []
    finally:
        try:
            cursor.close()
...
def atualizar_status_pedido(id_pedido, novo_status):
    try:
        ...
        conexao.commit()
        return True, str("✅ Status do pedido atualizado com sucesso!")
    except Error as erro:
...
def atualizar_pedido(id_pedido, id_ong, id_doador, id_intencao, status, data_criacao):
    try:
        ...
        cursor.execute(sql, valores)
        conexao.commit()
        return True, str("✅ Pedido atualizado com sucesso!")
    except Error as erro:
        return False, str(f"❌ Erro ao atualizar pedido: {str(erro).strip()[:200]}")
    finally:
        try:
            cursor.close()
            conexao.close()
...
def deletar_pedido(id_pedido):
    try:
        ...
        conexao.commit()
        if cursor.rowcount:
            return True, str("✅ Pedido deletado com sucesso!")
        else:
            return False, str("⚠️ Pedido não encontrado.")
    except Error as erro:
        return False, str(f"❌ Erro ao deletar pedido: {str(erro).strip()[:200]}")
    finally:
        try:
            cursor.close()
            conexao.close()
...
if submit_create:
    if not id_pedido or not id_ong or not id_doador or not id_intencao:
        feedback_create.warning(str("⚠️ Todos os campos são obrigatórios."))
    else:
...
if st.button("📥 Carregar pedidos"):
    ...
    if dados:
        feedback_read.success(str(f"✅ {len(dados)} pedidos encontrados."))
        st.table(dados)
    else:
        feedback_read.info(str("Nenhum pedido encontrado ou banco indisponível."))
...
if submit_update_all:
    if not id_pedido_upd or not id_ong_upd or not id_doador_upd or not id_intencao_upd:
        feedback_update_all.warning(str("⚠️ Todos os campos são obrigatórios."))
    else:
