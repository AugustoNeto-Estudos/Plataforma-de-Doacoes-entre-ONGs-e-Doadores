...
def atualizar_lista(id_lista, titulo=None, id_ong=None, status=None, descricao=None):
    try:
        ...
        if cursor.rowcount:
            ...
            return False, "⚠️ Lista não encontrada."
    except Error as erro:
        return False, str(erro)
    finally:
        if conexao:
...
if submit_create:
    if not id_lista or not titulo or not id_ong:
        feedback_create.warning("⚠️ Campos obrigatórios: ID da Lista, Título e ID da ONG.")
    else:
...
if submit_consulta:
    ...
    if lista:
        feedback_consulta.success("✅ Lista encontrada!")
        st.json(lista)
    else:
        feedback_consulta.warning("Lista não encontrada.")
