...
# CREATE - Doador
# ---------------------------
def cadastrar_doador(cpf, nome, email, contato, senha):
    conexao = None
    try:
        ...
        ("CPF_ID", nome, email, contato, senha) 
        VALUES (%s,%s,%s,%s,%s)
        """
        valores = (cpf, nome, email, contato, senha)
...
# ---------------------------
def atualizar_doador(cpf, nome, email, contato, senha):
    conexao = None
    try:
        conexao = criar_conexao()
        if not conexao:
        ...
        SET nome=%s, email=%s, contato=%s, senha=%s
        WHERE "CPF_ID" = %s
        """
        valores = (nome, email, contato, senha, cpf)
        cursor.execute(sql, valores)
        conexao.commit()
...
if submit_create:
    if not cpf or not nome or not email or not senha:
        ...
        sucesso, mensagem = cadastrar_doador(cpf, nome, email, contato, senha)
...
if submit_update:
    if not cpf_upd:
        ...
        sucesso, mensagem = atualizar_doador(cpf_upd, nome_upd, email_upd, contato_upd, senha_upd)
        mensagem = str(mensagem)
        if sucesso:
